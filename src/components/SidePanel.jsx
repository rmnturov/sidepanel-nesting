...
export default function SidePanel({ isOpen, onClose, children, level = 1, isLowerPanel = false, isBottomPanel = false, isBecomingNormal = false }) {
  ...
  return (
    ...
    >
      ...
      {/* Сама панель */}
      <div className={`side-panel ${isLowerPanel ? 'lower-panel' : ''} ${isBottomPanel ? 'bottom-panel' : ''} ${isBecomingNormal ? 'becoming-normal' : ''}`}>
        {children}
      </div>
    </div>
  )
